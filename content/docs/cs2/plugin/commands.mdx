...
| `.stop` | Ask to restore the current round. Both teams must type it. Only when `at_stop_command_available` is on. |
| `.gg` | Vote to give up the match. Only when `at_gg_enabled` is on. |
...
| `.forceready` | Mark a team ready. Only when `at_allow_force_ready` is on. |
...
Practice mode has more commands for grenades, bots and spawns, for example `.bot`, `.savenade <name>`, `.loadnade <name>`, `.rethrow`, `.showspawns` and `.noflash`. They work the same way as in the plugin Auto Tournament CS2 is forked from.
...
| Command | What it does |
| --- | --- |
| `at_loadmatch_url "<url>" ["<header>" "<value>"]` | Download and load a match config. The optional header is sent with the request. |
| `at_loadmatch <file>` | Load a match config from a file, relative to `csgo/`. |
| `at_clear_queued_match` | Drop a match that waits for the current series to reset. |
| `at_addplayer <steam64> <team1\|team2\|spec> "<name>"` | Add a player to the loaded match. |
| `at_removeplayer <steam64>` | Remove a player from all teams. |
...
| Command | What it does |
| --- | --- |
| `at_version` | Show the plugin version. |
| `at_match_report` | Build the match report as JSON and push it to the report URL. If there is no report URL, it prints the JSON. |
| `at_get_match_stats <matchId>` | Print the stats of a match from the plugin database as JSON. |
| `at_get_pending_events` | Show how many events wait in the retry queue. |
| `at_clear_event_queue` | Delete all waiting and failed events from the retry queue. |
| `at_check_for_updates` | Ask Steam if this CS2 server is up to date. |

### Backups

| Command | What it does |
| --- | --- |
| `at_listbackups <matchId>` | List round backups of a match. |
| `at_loadbackup <file>` | Restore a backup file. |
| `at_loadbackup_url <url>` | Restore a backup from a URL. |

### Config

| Command | What it does |
| --- | --- |
| `at_reload_config` | Run `cfg/AutoTournamentCS2/config.cfg` again. |
| `reload_admins` | Load `cfg/AutoTournamentCS2/admins.json` again. |
