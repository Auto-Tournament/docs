...
The release zip has one top folder, `AutoTournamentCS2-<version>/`. Copy what is **inside** that folder into the server's `game/csgo/` folder. Change `VERSION` to the [latest release](https://github.com/Auto-Tournament/cs2-plugin/releases/latest) and `CS2_DIR` to your server path:

```bash
VERSION=2.0.0
CS2_DIR=/home/steam/cs2
curl -L -o /tmp/AutoTournamentCS2.zip "https://github.com/Auto-Tournament/cs2-plugin/releases/download/v${VERSION}/AutoTournamentCS2-${VERSION}.zip"
unzip -q -o /tmp/AutoTournamentCS2.zip -d /tmp
cp -r "/tmp/AutoTournamentCS2-${VERSION}/." "${CS2_DIR}/game/csgo/"
```

You now have:

- `game/csgo/addons/counterstrikesharp/plugins/AutoTournamentCS2/AutoTournamentCS2.dll`
- `game/csgo/cfg/AutoTournamentCS2/`
...
Auto Tournament CS2 must be in the list. If it is missing, check that CounterStrikeSharp loads first.
...
Set the database in `game/csgo/cfg/AutoTournamentCS2/database.json` on each server:

```json title="game/csgo/cfg/AutoTournamentCS2/database.json"
{
  "DatabaseType": "MySQL",
  "MySqlHost": "192.168.1.20",
  "MySqlDatabase": "auto_tournament_cs2",
  "MySqlUsername": "auto_tournament_cs2",
...
Save your own `cfg/AutoTournamentCS2/database.json` and `cfg/AutoTournamentCS2/config.cfg` if you changed them. The zip contains default copies that overwrite them.
...
Copy your saved `database.json` and `config.cfg` back into `cfg/AutoTournamentCS2/`.
