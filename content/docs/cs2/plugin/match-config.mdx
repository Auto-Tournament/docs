...
```text
matchzy_loadmatch_url "https://tournament.example.com/api/matches/r1m1.json" "X-MatchZy-Token" "<SERVER_TOKEN>"
```

Auto Tournament's match config URL needs this header. The config holds the team rosters with Steam IDs, so Auto Tournament answers `401` without it. Auto Tournament adds the header itself when it loads a match. If you run the command by hand, add `"X-MatchZy-Token" "<SERVER_TOKEN>"` as above. Signed-in admins can open the URL in a browser.
