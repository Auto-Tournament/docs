---
title: API reference
description: The types and functions in plugin_api.h (API version 1.2), and the interfaces plugins share.
icon: BookOpen
---

The API is one C header, `core/include/readyup/plugin_api.h`. This page describes version 1.2. The header has the exact signatures and a comment on every member. When this page and the header differ, the header is right.
...
A plugin that requires 1.2 in `ru_plugin_info.api_version` can call everything. A plugin that requires an older minor must check `RU_API_HAS(api, member)` before it calls a newer member. The rules are on [Threading and ABI](/cs2/ready-up/plugin-development/threading-and-abi#versions).
...
| `set_admin_provider(self, fn, user)` | Decide who is an admin. One provider at a time. `fn` returns `1`, `0`, or `-1` for "no opinion". It can run on any thread. The match plugin registers one. |
