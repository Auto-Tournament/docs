...
<Callout title="Paths are moving" type="info">
  The core source is moving from `src/readyup/` to `core/src/`. This page names the current files. The move changes paths only, not the code.
</Callout>
...
Every feature lists the engine surface it needs in `src/readyup/features.cpp`. For example, `ready_hud` needs the GameFrame hook, `LegacyGameEventListener` and the game event manager.
...
The skins code uses its own set of functions (paints, knife subclass, models, bodygroups). They are described in `docs/skins-engine-surface.md` in the repo.

<Callout title="Planned" type="info">
  When skins moves to its own plugin, its entries move to `gamedata/engine-surface.skins.json`, which only ships with the skins package. A core without skins will then not look for them, and the matching API functions will return 0.
</Callout>
