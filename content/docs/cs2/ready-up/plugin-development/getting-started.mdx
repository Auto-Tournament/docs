...
<Callout title="In development" type="warn">
  The plugin API is at version 1.2. New minor versions only add functions, so plugins built for 1.2 keep working. Details on this page can still change before the first release.
</Callout>
...
The match flow, skins and the fleet link are all plugins built this way. This page walks through `hello`, the example plugin in the repo. It is plain C and about 200 lines.
...
| `core/include/readyup/*_iface.h` | Interfaces that plugins publish to each other and to the core. See [API reference](/cs2/ready-up/plugin-development/api-reference#interfaces). |
...
A reloaded plugin starts with no state, unless it kept some with `stash_put`. If the new file fails to load, the plugin stays unloaded. Put the old file back and reload again. See [What survives a reload](/cs2/ready-up/plugin-development/threading-and-abi#what-survives-a-reload).
