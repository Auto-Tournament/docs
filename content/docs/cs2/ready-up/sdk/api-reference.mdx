...
A plugin that requires 1.2 in `ru_plugin_info.api_version` can call everything. A plugin that requires an older minor must check `RU_API_HAS(api, member)` before it calls a newer member. The rules are on [Threading and ABI](/cs2/ready-up/sdk/threading-and-abi#versions).
