...
<Callout title="In development" type="warn">
  The SDK is at API version 1.2 and Ready Up is not released yet. New minor versions only add functions, so a plugin built today keeps working on later 1.x cores.
</Callout>
...
- **Interfaces between plugins.** A plugin can publish a struct of functions under a name, and other plugins can look it up. The match plugin publishes `readyup.match.v1` (`match_iface.h`). The fleet plugin publishes `readyup.fleet.v1` (`fleet_iface.h`), which other plugins use to talk to the Auto Tournament platform. Any plugin can add its own lines to `ru selftest`.
