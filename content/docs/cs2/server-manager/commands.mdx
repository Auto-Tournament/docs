...
| `csm monitor` | Run the update monitor once. It updates stopped servers and idle running servers. |
| `csm updates hold on\|off` | Stop or allow automatic updates by the monitor, for example during an event. Saved across reboots. |
| `csm updates status` | Show whether updates are on hold and the idle grace period. |
| `csm updates grace <minutes>` | How long a server must be idle before the monitor updates it. The default is 10. |
...
| `CSM_AT_SCOPE_PREFIX` | Prefix for the plugin scope name, instead of the hostname. |
