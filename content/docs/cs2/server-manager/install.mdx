...
`sudo csm` opens the menu. The installer is under **Install** > **Install or redeploy servers**. It asks:
...
Open these ports in your firewall. Players need UDP. Auto Tournament needs TCP on the game port for RCON.

## Add the servers to Auto Tournament

In Auto Tournament, add each server with the machine's IP, its game port and the RCON password. See [Connect a CS2 server](/getting-started/connect-server).

Get the public IP of the machine with:
