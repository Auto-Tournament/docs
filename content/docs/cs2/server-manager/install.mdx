...
- **Shared MySQL storage:** shared MySQL (the default) or one SQLite file per server. Shared MySQL keeps all match stats in one place and runs in a Docker container. You can also point it at your own MySQL server.
...
| `AT_DB_ENGINE` | `mysql` | `sqlite` for one SQLite file per server. |
| `AT_SKIP_DOCKER` | `0` | `1` to skip the MySQL container. |
