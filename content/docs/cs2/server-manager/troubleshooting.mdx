...
- **Auto Tournament CS2 per-server config (shared database)**: servers on one MySQL database that would overwrite each other's settings.
...
The servers share one MySQL database and overwrite each other's settings. Run `sudo csm doctor` and follow the steps under **Auto Tournament CS2 per-server config (shared database)**. See [Several servers on one database](/cs2/server-manager/configuration#several-servers-on-one-database).
