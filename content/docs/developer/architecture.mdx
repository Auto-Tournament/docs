...
- **From the server:** the plugin posts events to `/api/events` and match reports to `/api/events/report`, and downloads match configs from `/api/matches/<slug>.json`. Each request carries `SERVER_TOKEN` in the `X-MatchZy-Token` header. `matchEventHandler` applies the events.
