...
3. Check that MatchZy is loaded: run `css_plugins list` in the server console.
...
Without CS2 Server Manager, update the server with SteamCMD and restart it. Then click **Refresh Status**. Install the update monitor so this happens on its own, see [CS2 Server Manager](/modules/cs2/server-manager#automatic-updates).

### Plugin DB down

MatchZy Enhanced cannot reach **its own** database on the game server. This is not the Auto Tournament database.
...
The servers run different versions of MatchZy Enhanced. Update all of them to the same version:
...
Several servers share one MySQL database with an old MatchZy Enhanced, or without a scope name. See [Several servers and one database](/modules/cs2/matchzy-enhanced#several-servers-and-one-database).
