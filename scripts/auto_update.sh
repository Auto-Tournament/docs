...
# Auto-update docs.autotournament.gg without inbound ports.
# - Fetch origin/<branch>
...
REPO_DIR="${REPO_DIR:-/home/$USER/autotournament-docs}"
BRANCH="${BRANCH:-main}"
COMPOSE_DIR="${COMPOSE_DIR:-$REPO_DIR}"
LOCK_FILE="${LOCK_FILE:-/tmp/autotournament-docs-autoupdate.lock}"
...
DEPLOYED="$(docker inspect -f '{{ index .Config.Labels "org.opencontainers.image.revision" }}' autotournament-docs 2>/dev/null || true)"
if [[ -z "${DEPLOYED:-}" ]]; then
  DEPLOYED="$(docker exec autotournament-docs cat /app/BUILD_COMMIT 2>/dev/null | tr -d '\r\n' || true)"
fi
